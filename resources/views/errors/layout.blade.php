...
<head>
    ...
    <title>@yield('title', 'Có lỗi xảy ra') — {{ \App\Models\Setting::get('site_name', 'HocLieuTieuHoc') }}</title>
    @php
...
<body>
    ...
    <div style="text-align:center;padding:2rem;max-width:480px;width:100%;position:relative;z-index:1;">
        ...
        <div class="tiger-wrap" style="display:inline-block;margin-bottom:1rem;">
            <svg width="@yield('tiger-size', '140')" height="@yield('tiger-size', '148')" viewBox="0 0 160 170" xmlns="http://www.w3.org/2000/svg">
                {{-- Tail --}}
                <g class="tiger-tail">
        ...
        <h1 style="font-size:1.5rem;font-weight:800;color:#1f2937;margin:.25rem 0 .5rem;">@yield('heading', 'Có lỗi xảy ra')</h1>
        <p style="color:#6b7280;font-size:.95rem;margin-bottom:2rem;line-height:1.6;">@yield('message', 'Hổ cũng không biết chuyện gì xảy ra nữa...')</p>

        @hasSection('action-buttons')
            @yield('action-buttons')
        @else
        <div style="display:flex;gap:.75rem;justify-content:center;flex-wrap:wrap;">
            <a href="{{ url('/') }}" class="btn-home" style="display:inline-flex;align-items:center;gap:.5rem;padding:.75rem 1.5rem;background:#3a6b00;color:white;border-radius:1rem;font-weight:800;text-decoration:none;font-size:.95rem;box-shadow:0 4px 18px -4px rgba(58,107,0,.4);">
        ...
        <p style="margin-top:2rem;font-size:.75rem;color:#9ca3af;">
            <a href="{{ url('/') }}" style="color:#6b9e00;text-decoration:none;font-weight:700;">{{ $siteName }}</a>
